...
if ($action === 'bulk_upload_grades') {
    ...
    try {
        $stmtFindStudent = $conn->prepare("SELECT id, full_name FROM users WHERE school_id = ? AND role = 'student'");
...
if ($action === 'create_class') {
    ...
    $units = intval($input['units'] ?? 3); // Default to 3 units
    $teacher_id = $_SESSION['user_id'];
    ...
    $stmt = $conn->prepare("INSERT INTO classes (teacher_id, subject_code, subject_description, section, class_code, semester, units) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isssssi", $teacher_id, $subject_code, $subject_desc, $section, $class_code, $semester, $units);
...
if ($action === 'edit_class') {
    ...
    $stmt = $conn->prepare("UPDATE classes SET subject_code = ?, subject_description = ?, section = ?, semester = ?, units = ? WHERE id = ?");
    $stmt->bind_param("ssssii", $subject_code, $subject_desc, $section, $semester, $units, $class_id);
...
if ($action === 'join_class') {
    ...
    $stmt = $conn->prepare("SELECT id, subject_code, section FROM classes WHERE class_code = ?");
